...
- 2026-07: **New R11 crest logo + animated splash (visual-only)** — Replaced the header emblem with the Figma "R11" crest (`src/assets/royal11-logo*.png`, re-hosted locally) plus a thin gold accent bar; updated favicon/app-icons + title in `public/`. Added `SplashScreen.jsx` shown once on load (near-black #050102, radial vignette, pulsing cherry-red/gold glows, twinkling sparkles, centered crest, "PREMIUM SPORTS & GAMING" / "Virtual Coins Platform" taglines, iOS home indicator) that fades out after ~2.3s and hands off to Home (if logged in) or Login. No backend changes.
