...
## Backlog / Remaining
- **P1 (NEXT — Task 3): Manager & Zonal Manager weekly performance incentives.** SA sets per-person weekly `target_amount_inr` + `incentive_pct` (`incentive_targets`, one active per person). Measured vs total CONFIRMED downline deposit revenue that Sun–Sat week (Manager = deposits under their Admins; Zonal = all Managers+Admins in zone). If met, bonus = downline revenue × incentive_pct, paid out of the Super Admin's 70% share (NOT the Admin's cut). **User choice: credit immediately at week-end** (evaluate in the APScheduler `_daily_maintenance` when a week closes), idempotent `incentive:{user_id}:{week_start}`. UI: SA sets/edits target+pct on a Manager/ZM; the Manager/ZM sees their target + current-week downline total + progress.
