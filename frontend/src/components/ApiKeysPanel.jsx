...
const ResultLine = ({ result }) => {
  ...
  return (
    <div data-testid="key-test-result" className={`mt-3 flex items-start gap-2 rounded-xl px-3 py-2.5 text-xs font-semibold ${ok ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"}`}>
...
const BalanceCell = ({ balance }) => {
  if (!balance) return <span className="text-xs text-slate-400">Not available</span>;
  return (
  ...
  return (
    <div data-testid="apikeys-panel">
      ...
      <section className={`${CARD} p-6`}>
        ...
        <p className="mt-1 text-sm text-slate-400">Paste a provider key. We detect the provider, let you test it live, then store it encrypted.</p>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-[1fr_auto]">
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium text-slate-900 outline-none transition-colors placeholder:text-slate-400 focus:border-sky-400"
          />
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm font-semibold text-slate-900 outline-none focus:border-sky-400"
            >
              ...
              return (
                <motion.div key={k.id} layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="font-mono text-xs text-slate-400">•••• {k.key_last4}</p>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <button data-testid={`apikey-test-${k.id}`} onClick={() => testSaved(k.id)} disabled={!!busy}
                      className="flex items-center gap-1.5 rounded-xl border border-sky-200 px-3 py-2 text-xs font-bold text-sky-600 transition-colors hover:bg-sky-50 disabled:opacity-50">
                    ...
                    <button data-testid={`apikey-delete-${k.id}`} onClick={() => remove(k.id)} disabled={!!busy}
                      className="grid h-9 w-9 place-items-center rounded-xl bg-rose-50 text-rose-600 transition-colors hover:bg-rose-100 disabled:opacity-50">
