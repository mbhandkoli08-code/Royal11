  ...
  return (
    <div data-testid="managers-panel">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[860px] text-left text-sm" data-testid="managers-table">
              ...
              <tbody className="divide-y divide-white/5">
                {filtered.map((r) => (
                  <tr key={r.user.id} data-testid={`manager-row-${r.user.id}`} className="transition-colors hover:bg-white/[0.02]">
                    <td className={tdCls}>
                    ...
                    <td className={`${tdCls} text-[#d4af37]`}>{fmtCoins(r.authorized_quota)}</td>
      ...
      <Modal open={modal?.type === "fund"} onClose={() => setModal(null)} title={`Fund ${modal?.row?.user.display_name || ""}`} testid="fund-manager-modal">
        <div className="space-y-4">
          <p className="text-xs text-[#8c8385]">Credits real, spendable coins into this Manager’s wallet so they can allocate to Admins.</p>
      ...
      <Modal open={modal?.type === "quota"} onClose={() => setModal(null)} title={`Set quota — ${modal?.row?.user.display_name || ""}`} testid="quota-manager-modal">
        <div className="space-y-4">
          <p className="text-xs text-[#8c8385]">Caps how many coins this Manager may allocate out. Cannot be set below what’s already allocated ({fmtCoins(modal?.row?.allocated_out)}).</p>
