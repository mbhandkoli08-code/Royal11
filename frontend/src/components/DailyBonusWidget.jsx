...
// Daily Bonus HUD (server-authoritative). The claimable state, amount and the
// next-claim time all come from the server; the countdown is derived from the
// server clock (we correct for client/server skew) so it can't be spoofed.
export const DailyBonusWidget = ({ className = "", onClaimed }) => {
  ...
  const claim = async () => {
    ...
    try {
      ...
      setStatus(data);
      toast.success("Daily bonus claimed!", { description: `+${data.amount} bonus coins` });
      onClaimed?.();
    } catch (e) {
  ...
  return (
    <div data-testid="daily-bonus-widget"
      className={`w-44 rounded-2xl border border-[var(--r-gold,#c9a227)]/50 bg-black/70 p-3 text-center shadow-2xl backdrop-blur-md ${className}`}>
      ...
      {status.claimable ? (
        <>
          <p className="mt-1 text-[11px] text-white/70" data-testid="daily-bonus-amount">+{status.amount} coins ready</p>
        ...
        <>
          <p className="mt-1 flex items-center justify-center gap-1 text-[11px] text-emerald-300"><Check className="h-3 w-3" /> Claimed today</p>
          <p data-testid="daily-bonus-countdown" className="mt-1.5 font-mono text-lg font-bold tracking-wider text-white/90">{hh}:{mm}:{ss}</p>
